...
typedef struct {
    ...
    char type[30];    // 消费类别（对应同学代码的category）
    float amount;     // 消费金额
    char remark[50];  // 消费备注
} Record;
//##########################################################################
...
void showMainMenu() {
    ...
    do {
        ...
        switch (choice) {
            ...
            // 统计分析模块
            case 6: monthlySummary(); break;                // 杨（月度汇总） 
            case 7: calculateMonthlyCategoryRatio(); break; // 余（类别占比） 
            case 8: queryMaxMinConsumption(); break;        // 赵（最值查询） 
            // 退出
            case 9:
                saveRecordsToFile();
